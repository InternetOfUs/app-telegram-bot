...
class LogMessageHandler:
    ...
    def handle_textual_response(self, message: TextualResponse, user_id: str, response_to: str) -> messages.ResponseMessage:
        content = TextualContent(message.text)
        return messages.ResponseMessage(str(uuid.uuid4()), self.channel, user_id, self.project, content, response_to)

    def handle_rapid_answer_response(self, message: RapidAnswerResponse, user_id: str, response_to: str) -> messages.ResponseMessage:
        content = TextualContent(message.text)
    ...
    def handle_image_url_response(self, message: UrlImageResponse, user_id: str, response_to: str) -> messages.ResponseMessage:
        content = AttachmentContent(message.url)
        return messages.ResponseMessage(str(uuid.uuid4()), self.channel, user_id, self.project, content, response_to)

    def handle_telegram_carousel(self, message: TelegramCarouselResponse, user_id: str, response_to: str) -> messages.ResponseMessage:
    ...
    def handle_wenet_textual_message(self, message: TextualMessage, user_id: str) -> messages.NotificationMessage:
        content = TextualContent(f"{message.title} - {message.text}")
        return messages.NotificationMessage(str(uuid.uuid4()), self.channel, user_id, self.project, content)

    def handle_wenet_message(self, message: Message, user_id: str) -> messages.NotificationMessage:
        content = TextualContent(message.label)
        return messages.NotificationMessage(str(uuid.uuid4()), self.channel, user_id, self.project, content,
                                            metadata=message.attributes)

    def handle_authentication_event(self, message: WeNetAuthenticationEvent, user_id: str) -> messages.NotificationMessage:
    ...
    def create_response(self, message: ResponseMessage, user_id: str, response_to: str) -> messages.ResponseMessage:
        """
    ...
    def create_notification(self, message: Union[Message, WeNetAuthenticationEvent], user_id: str) -> messages.NotificationMessage:
        """
