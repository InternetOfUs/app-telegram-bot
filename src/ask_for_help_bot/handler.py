...
class AskForHelpHandler(WenetEventHandler, StateMixin):
    ...
    def action_best_answer_publish(self, incoming_event: IncomingSocialEvent, intent: str) -> OutgoingEvent:
        ...
        message_string = ""
        if len(message_answers) != 0:
            message_string_collected_answers = f"{self._translator.get_translation_instance(self.publication_language).with_text('collected_answers').translate()} \n\n"
            message_notification.append(TextualResponse(message_string_collected_answers))
            ...
            message_lower_part = f"{self._translator.get_translation_instance(self.publication_language).with_text('no_collected_answers').translate()} \n\n"
            message_notification.append(TextualResponse(message_lower_part))
